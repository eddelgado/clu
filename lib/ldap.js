...
var api = {
  ...
  addUserToDistroGroup: function(address, user, distro, callback) {
    ...
    api.ldapSearch(_filter, _attr, function(result) {
      log.verbose("LDAP addUserToDistro [", user, "], ", "[", distro, "]" );
      var client = ldap.createClient({
        url: "ldap://" + address
      });
      ...
      client.bind(ldapUser, ldapPassword, function(err) {
        ...
        /* Check to see if given distro is a group
         * sAMAccountType's :
         * 268435456 SAM_GROUP_OBJECT
         * 268435457 SAM_NON_SECURITY_GROUP_OBJECT
        */
        if (distro.sAMAccountType !== 268435456 || 268435457) {
          log.verbose("Given Distro is not a Group");
        }
        // Make sure user is not already in the given distro
        if (distro.member.indexOf(user) !== -1) {
          log.verbose("User is already in the Distro");
        }
        // TODO: Need to test to make sure that this appends the user to the
        //       member list, and does not replace all users on the list
        var _changeObject = {
          operation: 'add',
          modification: {
            member: user
        ...
        client.modify(result[0].dn, _change, function(err) {
          client.unbind();
          callback(err, address, user, distro);
        });
      });
    });
  },
